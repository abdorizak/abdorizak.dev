...
const formatImageUrl = (url?: string) => {
  if (!url) return '';
  if (url.startsWith('/')) return `https://abdorizak.dev${url}`;
  return url;
};
...
const buildDescriptionHtml = (post: PartialBlog): string => {
  ...
  if (post.link)
    ...
    description += `<b><a href="https://abdorizak.dev/blog/${post.slug}">Read more...</a></b><br/><br/>`;
...
const getAllPostRssData = (post: PartialBlog) => {
  ...
  return {
    title: post.title,
    url: post.link || `https://abdorizak.dev/blog/${post.slug}`,
    date: post.date,
...
const defaultChannel = {
  'atom:link': {
    _attr: {
      href: 'https://abdorizak.dev/feed.xml',
      rel: 'self',
